...
def fit_exp_data(theta_0, mcmc_algo="DE-MC"):
    ...
    def ln_params_prior(tau, c_inf, c_0, leak):
        """!
    ...
    my_mcmc = DeMcMpi(lnprob, theta_0, n_chains=comm.size*10, mpi_comm=comm,
                inflate=1e-1, ln_kwargs={'y_data': y_data, 't': t_data})
    my_mcmc.run_mcmc(2000 * 100)
    ...
    if comm.rank == 0:
        ...
        fig = corner.corner(samples, labels=["$\tau$", "$c_\infty$", "$c_0$", "l"])
        fig.savefig("exp_emcee_out.png")

    # view results
    print("=== Opti values by MCMC ===")
    ...
    if comm.rank == 0:
        ...
        plt.close()
        plt.scatter(xdata, ydata, s=2, alpha=0.5, c='r', label="data")
        i=0
        reduced_model = lambda t, tau, c_inf, c_0, set_leak: exp_c1_model_full(tau, c_inf, c_0, set_leak, t)
        for sample in chain:
            i+=1
            reduced_model = lambda t, tau, c_inf, c_0, set_leak: exp_c1_model_full(tau, c_inf, c_0, set_leak, t)
            plt.plot(xdata, reduced_model(xdata, *sample), lw=0.2, alpha=0.1, c='b')
            if i > 1000:
                break
        plt.plot(xdata, reduced_model(xdata, *theta_est), lw=0.5, alpha=0.8, label="MCMC avg", c='b')
        plt.ylabel("c_t")
        plt.xlabel("time")
        plt.legend()
        plt.savefig("mcmc_trajectories.png")
