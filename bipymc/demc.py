...
class DeMcMpi(DeMc):
    ...
    def _mcmc_run(self, n, **kwargs):
        ...
        def update_chain_pool(k, c_id, current_chain, prop_chain_pool, prop_chain_pool_ids):
            ...
            prop_vector += McmcChain.var_ball(epsilon ** 2.0, dim)
        ...
        def update_chain_pool_dream(k, c_id, current_chain, prop_chain_pool, prop_chain_pool_ids):
            # ensure the current chain is not in the proposal chain pool
            valid_pool_ids = np.array(range(len(prop_chain_pool)))
            if c_id in prop_chain_pool_ids:
                valid_idxs = (c_id != prop_chain_pool_ids)
                valid_pool_ids = valid_pool_ids[valid_idxs]
            del_pairs = kwargs.get("del_pairs", 40)
            del_pairs = np.min((del_pairs, int(np.floor(len(valid_pool_ids) / 2))))
            gamma_base = kwargs.get("gamma", 2.38 / np.sqrt(2. * del_pairs * dim))
            ...
            # Generate proposal vector
            eps_u = McmcChain.var_box(epsilon, dim)
            eps_n = McmcChain.var_ball(epsilon ** 2.0, dim)
            prop_vector = (np.ones(dim) + eps_u) * gamma * (mut_a_chain_state - mut_b_chain_state)
            prop_vector += eps_n

            # choose subset of dimensions to update
            # flip unfair coin for each dim
            cr_mask = np.random.choice([True, False], p=[cr_prob, 1.0 - cr_prob], size=dim)
        ...
        while j < int((n - self.n_chains) / self.comm.size):
            ...
            for i, current_chain in enumerate(self.am_chains):
                ...
                if c_id not in global_chain_a_ids:
                    continue
                j += 1
                update_chain_pool_dream(k_gen, c_id, current_chain, global_chain_b, global_chain_b_ids)
            ...
            for i, current_chain in enumerate(self.am_chains):
                ...
                if c_id not in global_chain_b_ids:
                    continue
                j += 1
                update_chain_pool_dream(k_gen, c_id, current_chain, global_chain_a, global_chain_a_ids)
